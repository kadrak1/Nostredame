...
QR-код ведёт на URL `https://domain.com/table/{table_id}`. Лендинг показывает номер стола и кнопку «Заказать кальян» — гость сразу переходит к конструктору кальяна (HookahBuilder из T-050) без лишних шагов.

## 2. User Stories

- **US-060-1**: Как гость за столом, я хочу отсканировать QR-код и заказать кальян через телефон, чтобы не ждать кальянщика.
- **US-060-2**: Как гость, я хочу заказать кальян без регистрации за минимальное число шагов.
...
### 3.2 Лендинг стола
- FR-060-06: Страница `/table/{table_id}` показывает: название заведения, номер стола, кнопку «Заказать кальян»
- FR-060-07: Кнопка «Заказать кальян» → переход к `/table/{table_id}/order` с `HookahBuilder`
- FR-060-08: Если стол не найден или неактивен — страница ошибки "Стол не найден"
- FR-060-09: Для авторизованных гостей (T-080) — показать блок "Ваш последний кальян" (слот для T-100)
...
- NFR-060-04: Rate limit по паре **(IP + table_id)**: не более 3 заказов за 15 минут. Это позволяет гостям за разными столами в одном заведении (общий WiFi) не блокировать друг друга — каждый стол имеет независимый счётчик.
...
- **URL**: `/api/orders`
- **Auth**: public
- **Rate limit**: 3 запроса/15 мин на (IP + table_id)
- **Request**:
```json
{
  "table_id": 3,
...
- **Route**: `/table/:tableId`
- **Содержание**: название заведения, номер стола, одна крупная кнопка «Заказать кальян»
- **Стиль**: тёмная тема, mobile-first; минималистичный экран — ничего лишнего между QR и заказом
...
- [ ] AC-2: Лендинг показывает номер стола и одну кнопку «Заказать кальян»
- [ ] AC-3: Нажатие кнопки открывает HookahBuilder на странице `/table/{id}/order`
- [ ] AC-4: Заказ через `POST /api/orders` создаётся с `source=qr_table`
- [ ] AC-5: Страница статуса показывает прогресс-бар и обновляется в реальном времени
- [ ] AC-6: WebSocket отправляет обновления при смене статуса
- [ ] AC-7: Несуществующий стол → страница ошибки 404
- [ ] AC-8: Rate limit работает — 4-й заказ с одной пары (IP + table_id) за 15 мин отклоняется с 429; гости за разными столами не блокируют друг друга
- [ ] AC-9: Админ может скачать QR-коды для всех столов одним ZIP-файлом
- [ ] AC-10: Mobile-first — лендинг и статус корректно отображаются на 320px+
...
| 5 | ✅ **Решено**: Проблема общего WiFi в заведении — rate limit по IP заблокировал бы всех гостей. Решение: rate limit по паре (IP + table_id), каждый стол независимо (NFR-060-04). |  |
