...
**PRD Requirement**: R6
**Status**: Planned
**Dependencies**: T-050 (hookah preorder), T-060 (QR ordering), T-080 (guest auth)
**Date**: 2026-03-06
...
Telegram-бот предоставляет альтернативный канал для бронирования столов и заказа кальянов. Гость использует бота через привычный мессенджер: диалоги с кнопками вместо форм, deep links с QR-кодов на столах. Бот работает на long polling, использует `python-telegram-bot v20+` (async) и обращается к backend API через HTTP — так бизнес-логика не дублируется.
...
- **US-070-2**: Как гость, я хочу заказать кальян через Telegram после сканирования QR на столе.
...
- FR-070-02: `/start table_{table_id}` — deep link с QR, сразу открывает конструктор кальяна для стола
- FR-070-03: `/book` — запуск ConversationHandler бронирования
- FR-070-04: `/order` — запуск ConversationHandler заказа (требует ввод номера стола)
- FR-070-05: `/status` — статус последнего активного заказа/брони
- FR-070-06: `/cancel` — отмена активного заказа/брони (с подтверждением)
- FR-070-07: `/help` — список команд
...
- FR-070-16: Шаг 1 — Крепость: 3 inline кнопки (🌿 Лёгкий / 🔥 Средний / 💪 Крепкий)
- FR-070-17: Шаг 2 — Табаки: постраничный список табаков (по 5 на страницу) с кнопками `[✓ Выбрать]`
- FR-070-18: Шаг 3 — Выбрано X табаков: кнопки "Добавить ещё" / "Готово"
- FR-070-19: Шаг 4 — Комментарий: ввод текстом или кнопка "Пропустить"
- FR-070-20: Шаг 5 — Подтверждение: сводка + "✅ Заказать"
- FR-070-21: После создания заказа — сообщение со статусом и обещанием уведомления при готовности

### 3.4 Deep Link с QR-кода
- FR-070-22: При переходе по `t.me/{bot}?start=table_3` бот парсит `table_id`
- FR-070-23: Сразу открывается ConversationHandler заказа (пропуская ввод номера стола)
- FR-070-24: Показывает: "Вы за столом №3. Выберите крепость кальяна:"
...
### 3.6 Аутентификация гостя в боте
...
| `GET /api/tables/{id}` | Информация о столе (для deep link) |
...
- States: STRENGTH, TOBACCOS, COMMENT, CONFIRM
- Поддержка deep link (передача `table_id` через context.user_data)
...
- [ ] AC-3: Deep link `t.me/{bot}?start=table_3` открывает заказ для стола 3
- [ ] AC-4: Заказ кальяна через бот создаётся с `source=telegram`
- [ ] AC-5: При подтверждении брони — гость получает уведомление в Telegram
- [ ] AC-6: При готовности кальяна — гость получает уведомление "Ваш кальян готов!"
- [ ] AC-7: Таймаут диалога 5 мин → `/cancel` срабатывает
- [ ] AC-8: `/cancel` с подтверждением отменяет активную бронь через API
- [ ] AC-9: Бот автоматически перезапускается при падении (`restart: unless-stopped`)
- [ ] AC-10: Все тексты вынесены в `messages.py`

## 10. Engineering Tickets / Тикеты

| Тикет | Название | Тип | Зависимости | Оценка |
|-------|----------|-----|-------------|--------|
| T-070 | Bot skeleton: main.py, api_client.py, messages.py, docker-compose сервис | backend | T-060, T-080 | M |
| T-071 | /start handler + главное меню + deep link парсинг | backend | T-070 | S |
