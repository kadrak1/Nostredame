...
Telegram + Web Push уведомления: подтверждение/отмена брони, напоминание за 2 часа, начало изготовления кальяна (`accepted`). Уведомление о готовности (`served`) — не нужно. APScheduler для напоминаний. Гость управляет настройками уведомлений.
